...
def extract_answer(info):
    ...
    for pred in prelim_predictions:
        ...
        orig_tokens = info["ori_tokens"][span_ids[pred.start_index]:span_ids[pred.end_index] + 1]
...
if __name__ == "__main__":
    ...
    for filename in tqdm(os.listdir(ensemble_data_files[0])):
        ...
        result[filename],nbest_result[filename] = extract_answer(data)

    json.dump(result,open(sys.argv[-1],"w"),ensure_ascii=False,indent=4)
    json.dump(nbest_result,open(sys.argv[-1] + '.nest',"w"),ensure_ascii=False,indent=4) 
