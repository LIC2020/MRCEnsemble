...
def extract_answer(info):
    ...
    for pred in prelim_predictions:
        ...
        final_text,start_index,end_index = get_final_text(tok_text, orig_text, do_lower_case=True, verbose_logging=True)
        break
...
def ensemble_logits(file_list,filename,weight):
...
#            assert sum(probs) == 1
            ...
            for index in range(len(data_list[i]["start_logits"])):
                data_list[i]["start_logits"][str(index)][2] = probs[index]*weight[i]
            ...
            for index in range(len(data_list[i]["end_logits"])):
                data_list[i]["end_logits"][str(index)] = probs[index]*weight[i]
...
def calc_weight(ensemble_data_files):
    weight = []
    try:
        file_f1 = json.load(open(f1_file))
    except:
        file_f1 = {}
    for f in ensemble_data_files:
        if f.endswith("/"):
            f = f[:-1]
        if f in file_f1:
            weight.append(float(file_f1[f]))
        else:
            w = input("f1 for {}:".format(f))
            weight.append(float(w))
            file_f1[f] = w
    json.dump(file_f1,open(f1_file,"w"),indent=4)

    for i,w in enumerate(weight):
        weight[i] = (w-71.5)/max(weight)
    print (weight)
    return weight
if __name__ == "__main__":
    ...
    weight = calc_weight(ensemble_data_files)
    
    for filename in os.listdir(ensemble_data_files[0]):
        data = ensemble_logits(ensemble_data_files,filename,weight)
        result[filename] = extract_answer(data)
