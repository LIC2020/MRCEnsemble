...
def ensemble_logits(file_list,weight):
...
#            assert sum(probs) == 1
            ...
            for index in range(len(data_list[i][qid]["start_logits"])):
                data_list[i][qid]["start_logits"][str(index)][2] = probs[index] * weight[i]
            ...
            for index in range(len(data_list[i][qid]["end_logits"])):
                data_list[i][qid]["end_logits"][str(index)] = probs[index]* weight[i]
...
def calc_weight(ensemble_data_files):
    ...
    for f in ensemble_data_files:
        if f in file_f1:
            weight.append(float(file_f1[f])*0.01)
        else:
            w = input("f1 for {}:".format(f))
            weight.append(float(w)*0.01)
            file_f1[f] = w
...
if __name__ == "__main__":
    ...
    data = ensemble_logits(ensemble_data_files,weight)
